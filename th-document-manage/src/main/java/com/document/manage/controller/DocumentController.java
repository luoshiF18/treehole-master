...
public class DocumentController {
    ...
    public EasyUIResult recycledJson(){
        ...
        List<Document> docList = documentServiceImpl.showRecycledDocument();
    ...
    //对外提供接口上传
    @PostMapping("/uploadPort/{describe}/{folderId}/{userId}")
    @ResponseBody
    public String uploadPort(@RequestPart("file") MultipartFile file, @PathVariable("describe") String describe, @PathVariable("folderId") String folderId, @PathVariable("userId") String userId) {
        ...
        files[0] = file;
        List<Document> list = documentServiceImpl.upload(files, folderId, describe.equals("") ? null : describe, userId);
        return list.get(0).getId();
    }
    //对外提供接口下载文档
    @PostMapping("/downloadPort/{docId}/{userId}")
    @ResponseBody
    public String downloadPort(@PathVariable("docId") String docId, @PathVariable("userId") String userId,HttpServletResponse response){
        //documentServiceImpl.downloadPort(docId, userId, response);
        //documentServiceImpl.download(docId,userId,response);
        String msg = "进啊来了";
        System.out.println("进啊来了");
        return msg;
    }
